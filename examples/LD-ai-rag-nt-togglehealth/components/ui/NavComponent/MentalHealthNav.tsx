...
import { NAV_ELEMENTS_VARIANT, MENTAL_HEALTH } from "@/utils/constants";
import { useRouter } from "next/router";
import LoginContext from "@/utils/contexts/login";
import Link from "next/link";
import LiveLogsContext from "@/utils/contexts/LiveLogsContext";

const MentalHealthNav = () => {
	...
	return (
		<NavWrapper>
			<>
				...
				<NavLogo
					srcHref={NAV_ELEMENTS_VARIANT[MENTAL_HEALTH]?.logoImg?.src}
					altText={MENTAL_HEALTH}
				/>
				...
				<NavbarLeftSideWrapper>
					<>
						{NAV_ELEMENTS_VARIANT[MENTAL_HEALTH]?.navLinks.map((navLink, index) => {
							return (
								<NavLinkButton
									text={navLink?.text}
									href={navLink?.href}
									navLinkColor={NAV_ELEMENTS_VARIANT[MENTAL_HEALTH]?.navLinkColor}
									index={index}
									key={index}
								/>
							);
				...
				<NavbarRightSideWrapper>
					<>
						{!isLoggedIn && (
							<>
								<NavbarSignUpButton
									backgroundColor="bg-mental-health-gradient-green-background hidden sm:block"
									onClick={() => {
...
export default MentalHealthNav;
