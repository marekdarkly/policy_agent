...
import { NAV_ELEMENTS_VARIANT, HEALTH } from "@/utils/constants";
	...
	return (
		<NavWrapper>
			<>
				...
				<NavLogo
					srcHref={NAV_ELEMENTS_VARIANT[HEALTH]?.logoImg?.src}
					altText={HEALTH}
				/>
				...
				<NavbarLeftSideWrapper>
					<>
						{NAV_ELEMENTS_VARIANT[HEALTH]?.navLinks.map((navLink, index) => {
							return (
								<NavLinkButton
									text={navLink?.text}
									href={navLink?.href}
									navLinkColor={NAV_ELEMENTS_VARIANT[HEALTH]?.navLinkColor}
									index={index}
									key={index}
								/>
							);
				...
				<NavbarRightSideWrapper>
					<>
						{!isLoggedIn && (
							<>
								<NavbarSignUpButton
									backgroundColor="bg-gradient-health hidden sm:block"
									onClick={() => {
